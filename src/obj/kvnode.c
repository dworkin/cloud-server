	   ...
	   object prev, string prevKey, string nextKey, object next)
{
	...
	if (type <= LEAF) {
		...
		if (found) {
			...
			key = keys[index];
			value = clone_object(KVNODE, accessKey, maxSize,
			...
			type = ROOT;
			values = ({
			    clone_object(KVNODE, accessKey, maxSize, keys,
					 values),
			    value
			});
			keys = ({ key });
		...
		if (sizeof(keys) > maxSize) {
		     ...
		     */
		    values = ({
			clone_object(KVNODE, accessKey, maxSize, keys, values),
			value
		    });
		    keys = ({ key });
		}
